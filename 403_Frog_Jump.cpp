...
public:
    bool canCross(vector<int>& stones) {
		...
		int n = stones.size();
		unordered_map<int, int> unit2index;
		vector<unordered_set<int>> steps(n, unordered_set<int>());

		//fill in unit2index
		for(int i=0; i<n; ++i){
			unit2index[stones[i]] = i;
		}
		...
		for(int i=0; i<n; ++i){
			for(auto k: steps[i]){
				...
				if(iter!=unit2index.end()){
					...
					steps[iter->second].insert(k);
					steps[iter->second].insert(k+1);
				}
			}
		}
