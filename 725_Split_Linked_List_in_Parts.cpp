...
public:
    vector<ListNode*> splitListToParts(ListNode* root, int k) {
		int list_len, max_group_size, num_of_plus_ones;
		...
		if(list_len%k){
			max_group_size = list_len / k + 1;
			num_of_plus_ones = list_len % k;
		}
		else{
			max_group_size = list_len / k;
			num_of_plus_ones = k;
		}
		...
		temp = root;
		for(int i=0; i<k; ++i){
			//get cur group size and create new list
